...
public class WeightedMatchingAlgorithm implements MatchingStrategy {
    ...
    private double calculateYearScore(Client currentClient, Client otherClient) {
        int yearOfStudy = currentClient.getPreferences().getYearOfStudy();
        int otherYear = otherClient.getPreferences().getYearOfStudy();
        double yearWeight = currentClient.getPreferences().getWeights().getOrDefault("YearOfStudy", THIRD);
